...
\author{Samuele Civale \and Matteo Zamponi \and Roberto Passante}
...
delays. The full-state critical-delay result is the gain-scaled form of the classical uniform-delay
consensus bound established by Olfati-Saber and Murray~\cite{olfati2004consensus}. The
neighbour-only communication-delay model is also related to the framework studied by Seuret,
Dimarogonas and Johansson~\cite{seuret2008consensus}, while the nonholonomic rendezvous
controller is motivated by the consensus-based rendezvous and formation laws for unicycle
robots proposed by Listmann, Masalawala and Adamy~\cite{listmann2009formation}.
\end{abstract}
...
\label{eq:taucrit}
\end{equation}
This expression is the direct gain-scaled version of the classical uniform-delay consensus
condition of Olfati-Saber and Murray~\cite{olfati2004consensus}. In their unit-gain formulation,
\(\dot x(t)=-Lx(t-\tau)\), average consensus on a fixed connected undirected graph is achieved
if and only if \(0\leq\tau<\pi/(2\lambda_{\max}(L))\). Introducing the scalar gain \(k\) replaces
each Laplacian eigenvalue \(\lambda_i\) by \(k\lambda_i\), which gives exactly
\eqref{eq:taucrit}.
...
\emph{IEEE Transactions on Automatic Control}, vol.~49, no.~9, pp.~1520--1533,
Sep. 2004, doi:~\href{https://doi.org/10.1109/TAC.2004.834113}{10.1109/TAC.2004.834113}.
