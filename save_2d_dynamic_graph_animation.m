function save_2d_dynamic_graph_animation(P, time, filename, title_str, xlim_vals, ylim_vals, num_frames, fixed_A, dynamic_radius, target)
%SAVE_2D_DYNAMIC_GRAPH_ANIMATION Animate 2D trajectories with fixed or distance-based edges.
if nargin < 10, target = []; end
...
for f = 1:length(frames)
    ...
    if ~isempty(target), plot(target(1), target(2), 's', 'MarkerSize', 8, 'LineWidth', 1.4); end
